...
.PHONY: runbot
runbot: start-db  # Запуск Telegram Bot с Uvicorn
...
.PHONY: runbot-existing-db
runbot-existing-db:  # Запуск Telegram Bot с существующей БД
	@echo -e "$(COLOR_YELLOW)Starting bot...$(COLOR_RESET)"
	@cd src && poetry run uvicorn core.asgi:application --reload && cd .. && \
	echo -e "$(COLOR_GREEN)Bot stopped$(COLOR_RESET)"
...
run-app: start-db # Запуск Django и Telegram бота
	@echo -e "$(COLOR_YELLOW)Starting bot...$(COLOR_RESET)"
	@cd src && poetry run uvicorn core.asgi:application --reload && cd .. && \
	echo -e "$(COLOR_GREEN)Bot stopped$(COLOR_RESET)"
